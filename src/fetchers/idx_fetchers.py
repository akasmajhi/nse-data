...
from src.fetchers.payloads import IDX_LIST
...
def fetch_idx_list(
    file_type: str = "INDEX",
    trading_date: str = datetime.strftime(datetime.today(), DATE_FMT),
):
    """Fetch all the index names along with OHLC for a given trading date."""
    logger.info(
        f"Fetching details for the indices with [{file_type = }], [{trading_date = }]"
    )
    df = pd.DataFrame()
    ...
    if idx_res.status_code == HTTPStatus.OK:
        ...
        with open(
            os.path.join(
                ...
                file_type.upper(),
                "raw/",
                f"{file_type.lower()}_{trading_date}_raw.csv",
            ),
            "w",
        ) as file:
        ...
        df = pd.read_csv(
            ...
            names=INDEX_LIST_COLUMNS,
            header=None,
            skiprows=INDEX_LIST_SKIPROWS,
        )
...
# TODO:
def fetch_idx_data(idx: str, start_dt: str | None, end_dt: str | None) -> pd.DataFrame:
    logger.debug(f"[{idx = }], [{start_dt = }], [{end_dt = }]")
    return pd.DataFrame()

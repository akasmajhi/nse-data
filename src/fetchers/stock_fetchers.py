...
from src.helpers.common import (
    get_stock_fetch_history,
    get_latest_history,
    register_failed_fetch,
    set_stock_fetch_history,
    compose_local_filename,
)


def get_stock_data_since_listing(
    stock_name: str, skip_current_year: bool = False
) -> pd.DataFrame:
    """
    ...
    logger.debug(
        f"[{stock_name = }], [{stock_listing_date = }], [{stock_listing_year = }], [{current_year = }]"
    )
...
def get_listing_date(stock_name: str) -> str:
    ...
    if stock_quote_res.status_code == HTTPStatus.OK:
        ...
        file_name = os.path.join(
            FILES_BASE_DIR, STOCK, "META", f"{stock_name.upper()}_meta.json"
        )
        with open(file_name, "w") as file:
            file.write(stock_quote_res.text)
        return read_listing_date_from_file(stock_name)
    logger.error(
        f"Something went wrong while getting listing date for: [{stock_name = }]"
    )
    ...
    if file_name and os.path.exists(file_name):
        ...
        try:
            with open(file_name, "r") as file:
                ...
                try:
                    logger.debug(
                        f"[{data['metadata']['listingDate'] = }] for [{stock_name = }]"
                    )
                    return data["metadata"]["listingDate"]
                except KeyError:
                    logger.error(
                        f"Listing date key not found in json for: [{stock_name = }]"
                    )
...
def fetch_stock_data_for_a_year(
    stock_name: str, year: int, skip_current_year: bool = False
):
    ...
    file_name = compose_local_filename(
        i_file_type=SUPPORTED_FILE_TYPES["STOCK"],
        i_trading_date="",
    ...
    if (year == current_year) and (not skip_current_year):
        ...
        if file_name and os.path.exists(file_name):  # File exists!
            ...
            latest_fetch_history = get_latest_history(
                get_stock_fetch_history(stock_name)
            )
            last_fetch_date = datetime.strptime(
                latest_fetch_history["fetch_date"], f"{DATE_FMT}:%H-%M-%S"
            )
            if last_fetch_date.strftime(DATE_FMT) == datetime.today().strftime(
                DATE_FMT
            ):
    ...
    # NOTE: Fetch for past years (non-current)
        ...
        if file_name and os.path.exists(
            file_name
        ):  # For past years don't fetch if file exists
            logger.info(
                f"For the stock: [{stock_name = }], file exists for: [{year = }]"
            )
            return
        else:
            from_date = datetime(year, 1, 1).strftime(DATE_FMT_1)
...
def fetch_stock(stock_name: str, from_date: str, to_date: str):
    ...
    try:
        ...
        logger.error(
            f"Timeout exception while fetching [{stock_name = }] for year [{from_date = }]"
        )
        register_failed_fetch(
            stock_name, from_date, to_date, "requests.exceptions.Timeout"
        )
    except requests.exceptions.TooManyRedirects:
        logger.error(
            f"Too many redirects fetching [{stock_name = }] for year [{from_date = }]"
        )
        register_failed_fetch(
            stock_name, from_date, to_date, "requests.exceptions.TooManyRedirects"
        )
    except requests.exceptions.ConnectionError as e:
        logger.error(
            f"Connection error fetching [{stock_name = }] for year [{from_date = }]"
        )
        register_failed_fetch(
            stock_name, from_date, to_date, "requests.exceptions.ConnectionError"
        )
        logger.error(e)
    year = from_date[-4:]
    file_name = compose_local_filename(
    ...
    if stock_res and stock_res.status_code == HTTPStatus.OK:
        if file_name:
            with open(file_name, "w", encoding="utf-8") as file:
                file.write(
                    stock_res.content.decode("utf-8")
                )  # HACK: Keep this in mind for CSVs
        ...
        logger.error(
            f"Error occured while fetching: [{stock_name = }]! for: [{year = }]"
        )
...
def read_stock_info_from_file(
    ...
    file_name = compose_local_filename(
        i_file_type=SUPPORTED_FILE_TYPES["META"],
...
def fetch_stock_info(stock: str, series: str = "EQ") -> dict:
    ...
    Note
    ____
    In the last week of may the stock into fetch API changed and
    now it requires the series name.
    """
    # NOTE: Assume that the stock name is valid, make a URL fetch
    ...
    """
    https://www.nseindia.com/api/NextApi/apiClient/GetQuoteApi?functionName=getSymbolData&marketType=N&series=EQ&symbol=20MICRONS
    """
    stock_quote_res = requests.get(
        url=NSE_STOCK_QUOTE_URL,
        headers=REQ_HEADER,
    ...
    if stock_quote_res.status_code == HTTPStatus.OK:
        ...
        if not os.path.isdir(meta_folder):
            logger.debug(
                f"META Folder: [{meta_folder = }] does not exist. Creating . . . "
            )
    ...
    logger.error(f"The URL that caused the error is: [{stock_quote_res.url}]")
    return dict()  # NOTE: This should be interpreted as an error condition


def read_market_cap_from_file(
    stock_name: str, trading_date: str = datetime.today().strftime(DATE_FMT)
) -> dict:
    """Reads the market cap from stored file. If it does not exist then the
    caller needs to issue a fetch-request to get the market cap from NSE.
    ...
    """
    logger.debug(
        f"Reading market cap for [{stock_name = }], [{trading_date = }] from file"
    )
    file_name = compose_local_filename(
        i_file_type=SUPPORTED_FILE_TYPES["MARKET_CAP"],
        i_trading_date=trading_date,
        i_stock_name=stock_name,
    )
    ...
    if file_name and os.path.exists(file_name) and stock_name not in MCAP_BLACKL_ISTED:
        ...
        try:
            with open(file_name, "r") as file:
                ...
                m_cap_dict["TOTAL_M_CAP"] = data["equityResponse"][0]["tradeInfo"][
                    "totalMarketCap"
                ]
                m_cap_dict["FF_M_CAP"] = data["equityResponse"][0]["tradeInfo"]["ffmc"]
                m_cap_dict["TRADING_DATE"] = trading_date
                # logger.debug(f"{m_cap_dict = }")
                return m_cap_dict
        except KeyError:
...
def fetch_market_cap(symbol: str, series: str = "EQ") -> dict:
    ...
    The name of the stock.
        series: str
    The series name is mandatory in the fetch from NSE
    Returns
    -------
        dict
    Dictionary containing the details (along with market cap)
    """
    # NOTE: There is no way to fetch historical mcap from NSE site.
    # So, stamp the fetch date onto the file name.
    fetch_date: str = datetime.now().strftime(DATE_FMT)
    logger.info(f"Fetching market cap for [{symbol = }], [{series = }]")
    dummy_res = dummy_request()
    ...
    logger.info(f"The stock_quote_res code is: [{stock_quote_res.status_code = }]")
    if stock_quote_res.status_code == HTTPStatus.OK:
        # NOTE: Store the file inside the MCAP_FOLDER
        file_name = compose_local_filename(
            i_file_type=SUPPORTED_FILE_TYPES["MARKET_CAP"],
            i_trading_date=fetch_date,
            i_stock_name=symbol,
        )
        if file_name:
            with open(file_name, "w") as file:
                file.write(stock_quote_res.content.decode("utf-8"))
            return read_market_cap_from_file(symbol)
        else:
            logger.error(f"Invalid [{file_name =}]")
    else:
        logger.error(
            f"HTTP Error occurred! [{stock_quote_res.status_code = }] for {stock_quote_res.url}"
        )
...
if __name__ == "__main__":
    ...
    # functionName=getSymbolData&marketType=N&series=EQ&symbol=INFY

    # fetch_market_cap("HDFCBANK")
    # logger.info(read_market_cap_from_file("20MICRONS"))
    # logger.info(fetch_stock_info("20MICRONS"))
    logger.info(fetch_stock_info("AFIL", series="BE"))

...
def get_stock_data_since_listing(stock_name: str) -> pd.DataFrame:
    ...
    logger.debug(f"stock_name: [{stock_name}], stock_listing_date: [{stock_listing_date}],\
            stock_listing_year: [{stock_listing_year}], current_year: [{current_year}]")
    
    for year in range(int(stock_listing_year), current_year+1):
        fetch_stock_data_for_a_year(stock_name, year)
    ...
    if os.path.exists(file_name):
        ...
        try:
            with open(file_name, "r") as file:
                data = json.load(file)
                logger.info(f"Listing Date: [{data['metadata']['listingDate']}] for [{stock_name}]")
...
def fetch_stock_data_for_a_year(stock_name: str, year: int):
    STOCK = SUPPORTED_FILE_TYPES["STOCK"]
    ...
    #NOTE: Fetch for past years (non-current)
        ...
        if os.path.exists(file_name): # For past years don't fetch if file exists
            logger.info(f"For the stock: [{stock_name}], file exists for the year: [{year}]")
            return
        else:
            from_date = datetime(year,1,1).strftime(DATE_FMT_1)

...
import analytics.gainers as gainers
from src.helpers.common import get_last_monday
...
def top_gainers(file_type: str = C.SUPPORTED_FILE_TYPES["BHAVCOPY"], 
    ...
    match file_type:
        case "INDEX" if file_type == C.SUPPORTED_FILE_TYPES["INDEX"]:
            return gainers.index_gainers(duration, start_date)
        ...
        case "STOCK" if file_type == C.SUPPORTED_FILE_TYPES["STOCK"]:
            match duration.upper():
                ...
                case "DAY" if duration.upper() == C.SUPPORTED_TIME_DURATIONS["DAY"]:
                    return gainers.daily_gainer(file_type=file_type.upper(),\
                                                gain_type=gain_type,\
                                                duration=duration,\
...
if __name__ == "__main__":
    ...
    data = top_gainers(file_type=C.SUPPORTED_FILE_TYPES["STOCK"], 
                       gain_type="PRICE",

...
bhav_data = pd.read_csv(
    os.path.join(BHAV_DATA_DIR, bhav_file), names=cols, header=None, skiprows=1
)
bhav_data["pct_change"] = (
    (bhav_data["close"] - bhav_data["previous_close"])
    / bhav_data["previous_close"]
    * 100
)
...
gainers = DataFrame(
    bhav_data[bhav_data.close > bhav_data.previous_close][required_symbols]
)
...
data: pd.DataFrame = top_gainers(
    file_type=SUPPORTED_FILE_TYPES["INDEX"],
...
daily_index_data[daily_index_data["30 D % CHNG 01-Aug-2025"] > 0].sort_values(
    by="30 D % CHNG 01-Aug-2025", ascending=False
)[
    ["INDEX", "30 D % CHNG 01-Aug-2025"]
]  # pyright: ignore

# NOTE: 1-Year Change
daily_index_data[
    daily_index_data["365 D % CHNG 30-Aug-2024"].astype(float, errors="ignore") > 0
].sort_values(by="30 D % CHNG 01-Aug-2025", ascending=False)[
    ["INDEX", "365 D % CHNG 30-Aug-2024"]
]  # pyright: ignore
...
# week_data_list = list()
week_data_df: pd.DataFrame = get_data(
    file_type="INDEX", start_date="01-Sep-2025", end_date="02-Sep-2025"
)
...
def daily_change():
    ...
    sys.path.append(
        "/home/akasmajhi/anaconda3/envs/nse-data/lib/python3.11/site-packages"
    )
